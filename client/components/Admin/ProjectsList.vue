<template>
  <section class="mt-10 border-t border-slate-200 pt-8">
    <header class="mb-4 flex items-center justify-between gap-4">
      ...
      <button type="button" @click="emit('refresh')" class="btn-secondary btn-sm">
        Rafraîchir
      </button>
    </header>
    ...
    <ul v-else class="space-y-3">
      <article v-for="project in projects" :key="project.id" class="rounded-xl border border-border-grey bg-light p-4">
          ...
          <div class="min-w-0 flex-1">
            ...
            <div v-if="project.technologies?.length" class="mt-3 flex flex-wrap gap-2">
              <span v-for="technology in project.technologies" :key="technology.id"
                class="inline-flex items-center gap-2 rounded-lg bg-light px-3 py-2 text-sm font-medium text-dark">
                <img v-if="getTechnologyIconUrl(technology)" :src="getTechnologyIconUrl(technology)"
                  :alt="technology.name" class="h-5 w-5 object-contain" />
...
const getProjectImage = (project) => {
  ...
  if (/^https?:\/\//.test(project.imagePath)) {
